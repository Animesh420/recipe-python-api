...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_existing_ingredients(self):
        ...
        ingredient_existing = Ingredient.objects.create(
            user=self.user, name='Paneer')
        payload = {
            'title': 'Palak Paneeer',
            'time_in_minutes': 60,
            'price': Decimal('4.50'),
            'ingredients': [{'name': 'Paneer'}, {'name': 'Palak'}]
        }
        res = self.client.post(RECIPE_URL, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.ingredients.count(), 2)
        self.assertIn(ingredient_existing, recipe.ingredients.all())
    ...
    def test_create_ingredient_on_update(self):
        ...
        recipe = create_recipe(user=self.user)
        payload = {'ingredients': [{'name': 'Honey'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = Ingredient.objects.get(user=self.user, name='Honey')
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        self.assertNotIn(bread, recipe.ingredients.all())
